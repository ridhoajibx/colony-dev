...
const PropertyFormUpdate = ({ token, items, id, isUpdate }: Props) => {
  ...
  const { property, pending, error, message } =
  ...
  useEffect(() => {
    let arr: any[] = [];
    if (valueState) {
      City.getCitiesOfState(valueState?.countryCode, valueState?.isoCode).map(
        (item: any) => {
          arr.push({
            ...item,
            value: item.name,
  ...
  useEffect(() => {
    if (search?.lat)
      setValue("gpsLatitude", search.lat), clearErrors("gpsLatitude");
    if (search?.lng)
      setValue("gpsLongitude", search.lng), clearErrors("gpsLongitude");
    if (search?.address)
      setValue("street", search.address), clearErrors("street");
  }, [search]);
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="w-full">
      ...
      <div className="w-full grid col-span-1 gap-4 tracking-wider mb-5 px-6">
        <div className="w-full px-2">
          <Cards className="w-full bg-white p-6 shadow-card rounded-xl">
            <div className="w-full grid col-span-1 sm:grid-cols-5 gap-4">
                      ...
                      {!files ? (
                        ...
                        <img
                          src={
                            imageStatus
                              ? files
                              : `${url}property/propertyLogo/${files}`
                          }
                          alt="logo"
                          className="w-full max-w-[200px] h-full min-h-[200px] object-cover object-center border border-gray shadow-card rounded-lg p-4"
                        />
              ...
              <label
                className="col-span-1 my-auto font-semibold"
                htmlFor="propertyLegalEntityName">
                Legal Entity Name
              </label>
              <div className="w-full col-span-4">
                <input
