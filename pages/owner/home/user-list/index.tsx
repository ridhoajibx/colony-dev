...
const DomainUserManagement = ({ pageProps }: Props) => {
  ...
  const { users, user, pending, error, message } =
    useAppSelector(selectDomainUser);
  ...
  useEffect(() => {
    if (token)
      dispatch(getDomainStructures({ params: filterRoles.queryObject, token }));
  }, [token, filterRoles]);
  ...
  return (
    <DomainLayouts
          ...
          <div className="w-full flex flex-col gap-4 p-4">
            ...
            <div className="w-full flex items-center gap-4">
              ...
              <div className="flex flex-col gap-2">
                <h3 className="font-semibold">
                  {formData?.domainStructure?.domain?.domainName || ""}
                </h3>
                ...
                <div className="w-full max-w-max flex items-center gap-2 border px-2 py-1 rounded-lg text-sm bg-primary text-white font-semibold">
                  <p>
                    {formData?.domainStructure?.domainStructureName
                      ? formData?.domainStructure?.domainStructureName
                      : "-"}
                  </p>
                </div>
      ...
      {/* modal form invite */}
      <Modal isOpen={isFormInvite} onClose={isCloseFormInvite} size="small">
        <Fragment>
          <ModalHeader
            isClose
