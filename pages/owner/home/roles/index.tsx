...
import {
  deleteDomainStructures,
  getDomainStructures,
  selectDomainStructures,
} from "../../../../redux/features/domain/domainStructure";
...
const DomainRolesManagement = ({ pageProps }: Props) => {
  ...
  const { domainStructures, pending, error, message } = useAppSelector(
    selectDomainStructures
  );
  ...
  const isOpenFormEdit = (items: DomainStructureData) => {
    ...
    items?.domainAccess?.forEach((x) => {
      ...
      domainAccess.push({
        domainAccess: {
          ...x,
          id: x.domainAccess.id,
          value: x.domainAccess.id,
          label: x.domainAccess.domainAccessName,
        },
      });
    });

    items?.domainStructureAcceses?.forEach((x: any) => {
      domainStructureAcceses.push({
        id: x.domainAccess.id,
        value: x.domainAccess.id,
        label: x.domainAccess.domainAccessName,
      });
    ...
    items?.domainStructureAccessGroups?.forEach((x: any) => {
      domainAccessGroupAcceses.push({
        id: x.domainAccessGroup.id,
        value: x.domainAccessGroup.id,
        label: x.domainAccessGroup.domainAccessGroupName,
        domainAccessGroupAcceses: domainAccess,
      });
    });
    ...
    newObj = {
      ...items,
      id: items.id,
      domainAccess: domainStructureAcceses?.filter(
        (v, i, a) => a.findIndex((v2) => v2.id === v.id) === i
      ),
      domainStructureAccessGroups: domainAccessGroupAcceses?.filter(
        (v, i, a) => a.findIndex((v2) => v2.id === v.id) === i
      ),
    };
    setFormData(newObj);
  ...
  const columns = useMemo<ColumnDef<DomainStructureData, any>[]>(
    () => [
      ...
      },
      {
        ...
        header: (info) => <div className="uppercase">Roles Name</div>,
        cell: ({ getValue, row }) => {
          return (
      ...
      },
      {
        ...
        header: (info) => <div className="uppercase">Access Group</div>,
        cell: ({ getValue, row }) => {
          let access = getValue();
          let index: any[] = [0, 1, 2];
            ...
            <div className="w-full flex flex-col cursor-pointer tracking-wider">
              {access?.length == 0
                ...
                ? index?.map((acc: any, i: any) => {
                    return (
                      <div key={i}>
                        {access[acc]?.domainAccessGroup?.domainAccessGroupName},
                      </div>
                    );
                  })
                : access?.map((acc: any, i: any) => {
                    return (
                      <div key={i}>
                        {acc?.domainAccessGroup?.domainAccessGroupName}
                        {lastIndex == i ? "" : ","}
                      </div>
                    );
                  })}
  ...
  const filters = useMemo(() => {
    ...
    const search: any = {
      $and: [
        {
          $or: [{ domainStructureName: { $contL: query?.search } }],
        },
      ],
    };
  ...
  useEffect(() => {
    ...
    if (data || data?.length > 0) {
      ...
      data?.map((item: DomainStructureData) => {
        arr.push({
          ...item,
          domainAccess: item?.domainStructureAccessGroups?.reduce(
            (r: any, d: any) => {
  ...
  useEffect(() => {
    if (token)
      dispatch(
        getDomainAccessGroup({ params: filterAccess.queryObject, token })
      );
  }, [token, filters]);
