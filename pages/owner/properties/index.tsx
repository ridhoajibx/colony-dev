...
const DomainProperty = ({ pageProps }: Props) => {
            ...
            <Link
              ...
              <div className="w-full lg:w-1/5 text-lg font-semibold">
                <img
                  src={
                    logo
                      ? `${url}property/propertyLogo/${logo}`
                      : "../image/no-image.jpeg"
                  }
                  alt=""
                  className="w-full max-w-[150px] object-cover object-center m-auto"
                />
              </div>
                  ...
                  <div className="w-full flex items-center gap-2 justify-between">
                    ...
                    <div
                      className={
                        !status
                          ? "hidden"
                          : "p-1.5 bg-boxdark text-white text-sm rounded-lg"
                      }>
                      {status}
                    </div>
                  </div>
  ...
  const filters = useMemo(() => {
    ...
    if (query?.sort)
      qb.sortBy({
        field: "propertyName" || "propertyDescription",
        order: query.sort as QuerySortOperator,
      });
