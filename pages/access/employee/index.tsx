  ...
  const goToPropertyAccess = (id: number) => {
    dispatch(
      webPropertyAccess({
        ...
        token,
        callback: () =>
          router.push({ pathname: "/employee/building-management" }),
      })
    );
  };
  ...
  const Property = (props: any) => {
    ...
    return (
      <button
        ...
        <div className="w-full h-full hidden lg:flex justify-start lg:w-[10%] m-auto">
                ...
                <div className="w-full h-full flex flex-col gap-4 ">
                  {properties?.length > 0 ? (
                    properties?.map((item: any, index: any) => {
                      return <Property key={index} items={item} />;
                    })
                  ) : (
                    <div className="w-full">
