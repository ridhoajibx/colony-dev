...
import { uploadParkingLot } from "../../../../../redux/features/building-management/parking/parkingLotReducers";
...
import {
  ...
  selectParkingTransactionManagement,
} from "../../../../../redux/features/building-management/parking/parkingTransactionReducers";
...
const ParkingTransaction = ({ pageProps }: Props) => {
  ...
  const { parkingTransactions, pending } = useAppSelector(
    selectParkingTransactionManagement
  );
  ...
  // data-table
  const [dataTable, setDataTable] = useState<ParkingTransactionProps[]>([]);
  ...
  useEffect(() => {
    if (token)
      dispatch(getParkingTransactions({ token, params: filters.queryObject }));
  }, [token, filters]);

  useEffect(() => {
    let newArr: any[] = [];
    const { data, pageCount, total } = parkingTransactions;
  ...
  // column
  const columns = useMemo<ColumnDef<ParkingTransactionProps, any>[]>(
    () => [
      {
        id: "select",
        header: ({ table }) => {
      ...
      },
      {
        ...
        header: (info) => <div className="uppercase">Duration</div>,
        cell: ({ getValue, row }) => {
      ...
      },
      {
        accessorKey: "income",
        cell: ({ row, getValue }) => {
          return <div className="">{getValue() || "Rp. 0"}</div>;
        },
        header: (props) => <div className="w-full uppercase">Income</div>,
        footer: (props) => props.column.id,
        enableColumnFilter: false,
      },
      {
        ...
        header: (props) => (
          <div className="w-full text-center uppercase">Pass No.</div>
        ),
        footer: (props) => props.column.id,
        enableColumnFilter: false,
      },
  ...
  const onDownloadTemplate = async () => {
    ...
    await axios({
      url: `parkingLot/transaction/template`,
      method: "GET",
      responseType: "blob",
      headers: {
  ...
  const onDownload = async () => {
    ...
    let date = new Date();
    await axios({
      url: `parkingLot/transaction/download`,
      method: "GET",
      responseType: "blob",
      headers: {
        "Content-Type": "application/json",
    ...
    dispatch(
      uploadParkingLot({
        ...
        isSuccess: () => {
          ...
          onCloseUpload();
          dispatch(
            getParkingTransactions({ token, params: filters.queryObject })
          );
        },
        isError: () => {
