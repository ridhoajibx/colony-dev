...
import {
  getAccessCardTransactions,
  selectAccessCardTransactionManagement,
  uploadAccessCardTransaction,
} from "../../../../../redux/features/building-management/access/accessCardTransactionReducers";
...
const AccessCardTransaction = ({ pageProps }: Props) => {
  ...
  const { accessCardTransactions, pending } = useAppSelector(
    selectAccessCardTransactionManagement
  );
  ...
  // data-table
  const [dataTable, setDataTable] = useState<AccessCardTransactionProps[]>([]);
  ...
  }, [pages, limit, search, sort, startDate, endDate]);
      ...
      $and: [
        ...
        },
        {
          $or: [
            ...
            { "accessCard.cardNumber": { $contL: query?.search } },
            { "accessCard.unit.unitName": { $contL: query?.search } },
          ],
        },
      ],
    };
  ...
  useEffect(() => {
    if (token)
      dispatch(
        getAccessCardTransactions({ token, params: filters.queryObject })
      );
  }, [token, filters]);

  useEffect(() => {
    let newArr: any[] = [];
    const { data, pageCount, total } = accessCardTransactions;
  ...
  // column
  const columns = useMemo<ColumnDef<AccessCardTransactionProps, any>[]>(
    () => [
      // {
      //   id: "select",
      //   header: ({ table }) => {
      ...
      // },
      {
        ...
        header: (props) => <div className="w-full uppercase">Active Date</div>,
        footer: (props) => props.column.id,
        enableColumnFilter: false,
      },
      {
        accessorKey: "accessCard.cardHolder",
      ...
      {
        accessorKey: "accessCard.cardNumber",
      ...
      },
      {
        ...
        header: (props) => (
          <div className="w-full uppercase">Controller Address</div>
        ),
        footer: (props) => props.column.id,
        enableColumnFilter: false,
      },
      {
        ...
        header: (props) => (
          <div className="w-full uppercase">Controller Name</div>
        ),
        footer: (props) => props.column.id,
        enableColumnFilter: false,
      },
  ...
  const onDownloadTemplate = async () => {
    ...
    await axios({
      url: `accessCard/history/template`,
      method: "GET",
      responseType: "blob",
      headers: {
      ...
      .then((response) => {
        ...
        link.href = url;
        link.setAttribute("download", `AccessCard-Transaction-Template.xlsx`);
  ...
  const onDownload = async () => {
    ...
    let date = new Date();
    await axios({
      url: `accessCard/history/download`,
      method: "GET",
      responseType: "blob",
      headers: {
        "Content-Type": "application/json",
...
export default AccessCardTransaction;
